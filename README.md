...
Open <http://localhost:5173>, then open **Settings (⌘,)** and pick an LLM provider — paste your API key and choose a model there. (Exporting `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GEMINI_API_KEY` / `OPENROUTER_API_KEY` before launch works too, as a fallback.) The API server runs on `:4321`; the Vite dev server proxies `/api` to it.

> **Prerequisites:** Node `>=22 <23` (a `.nvmrc` pins `22` — run `nvm use`) and pnpm `10.20.x`. The simplest way to get the right pnpm is `corepack enable`, which honours the `packageManager` pin in `package.json`. Check with `node -v` and `pnpm -v`. Install: [Node via nvm](https://github.com/nvm-sh/nvm#installing-and-updating) (or [direct](https://nodejs.org/en/download)), [pnpm/corepack](https://pnpm.io/installation), [Git](https://git-scm.com/downloads).
...
- **Wiki (⌘2)** — pages per person / project / organisation / concept / place / decision, written and continuously rewritten by the LLM from accumulated facts. You never edit them. Each page shows the external sources it draws on as chips — a connector's items collapse into one chip with a count (hover for the list), files appear individually — and an on-demand **connections** graph of the entity's neighbours. Toggle the index's **Graph** view for a force-directed map of every entity and the relationships between them, coloured by type — pan, zoom, hover to trace a node's connections, and click any node to open its page.
- **Sources (⌘3)** — the browse companion to the Wiki: every locally-indexed connector item — a contact, calendar event, task, or email — as its own first-class entry, grouped by kind, each linking out to the original and to the wiki entities it feeds. Where the Wiki shows the synthesised pages, Sources shows the raw items behind them.
...
- **Connectors (Settings → Connectors)** — sync external accounts so the people, meetings, and correspondence in your life become knowledge automatically. Each connector is a self-contained plugin; **Google** is the reference one, grouped under a single connector with a switch per service: **Contacts** (People API) turn into person entities with their details, **Calendar** events become entities linking the people you met with, **Gmail** metadata maps who you correspond with, and **Tasks** are indexed and writable (MeOS can create tasks). OAuth is loopback + PKCE against your _own_ Google Cloud "Desktop app" client, so tokens never pass through anyone else; one account-wide control sets the sync interval and how far back to cover, and syncs run incrementally (sync tokens / history IDs, not full re-pulls). Contact details and email metadata stay private — searchable and browsable under **Sources**, but kept out of the git-synced wiki. New connectors are scaffolded with `pnpm connector:new <id>` (see [`docs/connectors.md`](docs/connectors.md)).
...
- **Settings (⌘,)** — pick the folders MeOS should watch (native folder picker in the desktop app), choose light / dark / system appearance, and pick the LLM provider (Anthropic, OpenAI, Google, OpenRouter, or a local OpenAI-compatible server like LM Studio — paste an API key or point at a local endpoint, choose a model; applies immediately, no restart). Everything readable in watched folders is absorbed automatically — new files and edits alike, across restarts. Supported types: notes & text (`.md`, `.markdown`, `.txt`, `.text`, `.org`), data & code (`.csv`, `.json`, `.yaml`/`.yml`, `.toml`, `.ini`, `.sql`, `.log`), documents (`.pdf`, `.docx`, `.rtf`, `.odt`, `.html`/`.htm`), spreadsheets (`.xlsx`, `.xls`, `.ods`), presentations (`.pptx`), email (`.eml`, `.mbox`), notebooks (`.ipynb`), and images (`.png`, `.jpg`/`.jpeg`, `.gif`, `.webp`). Change detection is content-based: a fast mtime + size check backed by a SHA-256 content hash, so each distinct version of a file is processed exactly once and a metadata-only touch (cloud re-download, backup restore) is skipped without re-running the LLM. Images are read by the LLM (text transcribed, content described) and absorbed like any note. Your files are never moved or modified.
- **⌘K** — jump to any view or wiki page from the keyboard.

## Configuration

The LLM provider, model, and API key are defined in **Settings** only (persisted in `data/meos.db`) — Anthropic (default), OpenAI, Google, OpenRouter, or a local OpenAI-compatible server (LM Studio, llama.cpp, Ollama's `/v1`) for fully local operation — point at its endpoint (e.g. `http://localhost:1234/v1`), no API key, nothing leaves the machine. Environment keys (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`, `OPENROUTER_API_KEY`) act as fallbacks when no key was pasted in Settings; `MEOS_LLM_PROVIDER` overrides the provider per-run.

An optional `meos.config.json` at the repo root overrides infrastructure config only. It is **not present by default** — built-in defaults apply unless you create it, and every field is optional. Notable:
...
A pnpm monorepo. Four packages make up the running app: `core` (runtime-agnostic domain logic — ingestion, knowledge store, extraction, memory, wiki writer, retrieval/chat, connectors, vault), `server` (Fastify API + background workers that wire `core` into one local process), `web` (React + Vite UI that talks to the server over a typed HTTP boundary), and `desktop` (a Tauri 2 shell owning the native window and server lifecycle). Two more support them: `contracts` (the shared Zod/TypeScript types that define the web↔server HTTP boundary) and `wiki-mcp` (an MCP server that exposes the generated wiki to external coding agents). MeOS runs local-first and essentially single-process, with strict one-directional package boundaries (`desktop → server → core`, `web → HTTP → server`). Tests run without any LLM or network: `pnpm test` (LLM calls are stubbed behind the `LlmClient` interface).
...
| Dependency                                                                                                                                                         | Why                                                                                                                    |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| `ai` (Vercel AI SDK) + `@ai-sdk/anthropic` / `@ai-sdk/openai` / `@ai-sdk/google` (the OpenAI client also drives OpenRouter and any local OpenAI-compatible server) | One client, every provider — unified completion / structured-output / streaming / tool-use behind a single `LlmClient` |
