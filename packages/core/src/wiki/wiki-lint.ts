...
export function lintPage(store: KnowledgeStore, entityId: number, body: string): PageLintResult {
  ...
  if (body.trim().length === 0) {
    return { entityId, quality: 0, issues: [{ code: "empty", severity: "review", detail: "page has no prose" }] };
  }
  ...
  if (broken.length > 0) {
    issues.push({ code: "broken_link", severity: "auto", detail: `unknown links: ${broken.join(", ")}` });
  }
  ...
  if (observations.length > 0 && grounded === 0) {
    issues.push({ code: "missing_citations", severity: "review", detail: "no claim cites a source" });
  ...
  if (vagueHits >= 4) {
    issues.push({ code: "vague_claims", severity: "review", detail: `${vagueHits} vague phrasings` });
  }

  // Score: start at 1, subtract weighted penalties, clamp to [0,1].
  const meanConfidence =
    observations.length === 0 ? 0 : observations.reduce((sum, o) => sum + o.confidence, 0) / observations.length;
  let quality = 1;
